...
<nav
  ...
  style={{
    ...
    width: "calc(100% - 3rem)",
    maxWidth: "1400px",
    borderRadius: mobileMenuOpen ? "1rem" : "9999px", // instant swap
  }}
>
  <div className="flex flex-col md:flex-row items-center px-8 py-4">
    ...
    {mobileMenuOpen && (
      ...
      <div className="flex-grow pt-32 px-4 sm:px-10 md:px-20 pb-20">
        ...
        {/* Blog Cards with Refined Animations */}
        {/* Blog Cards with Consistent Heights */}
<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-12">
  {blogPosts.map((post, index) => (
    ...
    >
      <Link
        href={`/blog/${post.slug}`}
        className="border border-[#0D1321] rounded-lg shadow hover:shadow-xl bg-[#FFFBEE] transition-all p-4 w-full h-full flex flex-col" // Added h-full and flex-col
      >
        ...
        <div className="flex-grow flex flex-col">
          <h3 className="text-xl md:text-2xl font-semibold font-[playfair_display] text-center mb-1">
            {post.title}
          </h3>
          <p className="text-sm text-gray-500 text-center">{post.date}</p>
          <p className="text-[#0D1321] text-base font-[DM_Sans] line-clamp-3 mt-2 flex-grow">
            {post.excerpt}
          </p>
        </div>
      </Link>
      ...
      {/* Footer */}
<footer className="bg-[#0D1321] text-white w-full py-6 mt-10">
